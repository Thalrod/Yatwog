...
from game.world.renderer import Renderer, Mode
...
class Map():
    def __init__(self):
        ...
        self.noiseMap = MapGenerator(self.mapWidth, self.mapHeight, self.seed, self.scale, self.octaves, self.persistence, self.lacunarity, self.offset).generate()
        
    def loadMap(self):
        self.maps = Renderer(self.noiseMap, self.regions).render(self.RenderMode)
        return self.maps
